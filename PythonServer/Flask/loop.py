...
hist = []
#対応表
taio = [[1,3,4],[3,4,5],[1,4,5],[0,4,6,7],[1,3,5,7],[2,4,7,8],[3,4,7],[4,8,6],[4,5,7]]
...
def ser_to_unity(ser):
    global num_funda
    global send_u_array
    print("int(ser.decode('utf-8'))=",int(ser.decode('utf-8')))
    
    if len(send_u_array[int(ser.decode('utf-8'))])==0 and send_u_array != ['']*9:
        print("other one")
        return send_u_array
...
#            same_del = [i for i in same if i not in hist]
        print(same)
        same = list(filter(lambda str:str != '', same))
#            print("same_del=" ,same_del)
        j=0
        ran_array = random.sample(same,len(book_array))
        for i in book_array:
            ran = int(ran_array[j])
            print("ran=",ran)
            j=j+1
...
#        if len(s)>0 and s!= prev_stat and stat[int(ser.decode('utf-8'))] != 0:
        if len(s)>0:
            ...
            if s!= prev_stat:
                print("s!=prev_stat")
                c = ser_to_unity(s)
                file_path_2 = os.path.join(file_path,data["ongen"][int(stat[int(s.decode('utf-8'))])-1])
                read_aloud(file_path_2)
